...
- Add entries under `## [Unreleased]`, in an `### Added` / `### Changed` /
...
- To release: rename `[Unreleased]` to the version with an ISO date, bump per
  SemVer (breaking → major, new command or entry → minor, fix → patch), and tag
  the commit `vX.Y.Z`.
- Purely internal changes with no user-visible effect need no entry. When
  unsure, add one.

### Record the commit message with the entry

Commits here are usually made from GitHub Desktop, which has a **Summary** field
and a **Description** field and cannot read a commit template. So every set of
`[Unreleased]` entries carries the message that will ship it, in a
`### Commit message` block sitting **directly under the version heading** —
above `### Added` and the rest, not at the bottom, since it is the part most
often needed:
...
Update the block whenever the entries below it change; if a release ends up
spanning several commits, keep one block per commit, newest first. On release the
block travels down into the version section along with its entries, staying
directly beneath that version's heading.
